...
class WarehouseController extends Controller
{
    ...
    function mapdemo(Request $request): View|Factory|Application
    {
        $warehouses = $this->warehouse->orderBy('id', 'desc')->first();
        if (!empty($warehouses) && ($warehouses->id < 9)) {
            $prevId = ($warehouses && $warehouses->id) ? '0' . $warehouses->id + 1 : 1;
        ...
        return view('admin-views.warehouse.map_demo', compact('prevId'));
    }
    ...
    function store(Request $request): RedirectResponse
    {
        ...
        if (
            !empty($request->pre_order_open_time) && is_array($request->pre_order_open_time) &&
            !empty(array_filter($request->pre_order_open_time, function ($value) {
                return $value !== null;
            })) &&
            !empty($request->pre_order_close_time) && is_array($request->pre_order_close_time) &&
            !empty(array_filter($request->pre_order_close_time, function ($value) {
                return $value !== null;
            }))
        ) {
            $pre_order = [];
