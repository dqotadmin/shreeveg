...
<div class="content container-fluid">
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form action="{{route('admin.business-settings.groups.store')}}" method="post" enctype="multipart/form-data" class="needs-validation form_customer g-2" novalidate>
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="row g-3">
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                <div class="form-group mb-0">
                                    <label class="input-label" for="{{translate('description')}}">{{translate('description')}}</label>
                ...
                <tr>
                    ...
                    <td>
                        <label class="toggle-switch my-0">
                            <input type="checkbox" onclick="status_change_alert('{{ route('admin.business-settings.groups.status', [$row->id, $row->status ? 0 : 1]) }}', '{{ $row->status? translate('you_want_to_disable_this_group'): translate('you_want_to_active_this_group') }}', event)" class="toggle-switch-input" id="stocksCheckbox{{ $row->id }}" {{ $row->status ? 'checked' : '' }}>
                    ...
                    <td>
                        ...
                        <div class="btn--container justify-content-center">
                            <a class="action-btn" href="{{route('admin.business-settings.groups.edit',[$row['id']])}}">
                                <i class="tio-edit"></i></a>
                            <a class="action-btn btn--danger btn-outline-danger" href="javascript:" onclick="form_alert('banner-{{$row['id']}}','{{ translate("Want to delete this") }}')">
                                <i class="tio-delete-outlined"></i>
                            </a>
                        </div>
                        <form action="{{route('admin.business-settings.groups.delete',[$row['id']])}}" method="post" id="banner-{{$row['id']}}">
                            @csrf @method('delete')
                        </form>
                        <!-- End Dropdown -->
                    </td>
