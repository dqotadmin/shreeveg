<div class="modal-body position-relative">
    ...
    <div class="modal--media">
        ...
        <div class="details">
            ...
            @if ($product->tax)
                <div class="mb-3 text-dark">
                    <span class="h3 font-weight-normal text-accent mr-1">Tax: {{ $product->tax }}%
                    </span>
                </div>
            @endif
            @if ($discount > 0)
                <div class="mb-3 text-dark">
                    ...
                    <strong id="set-discount-amount">{{ $discount }}</strong>
                </div>
            @endif
    ...
    <div class="row pt-2">
        <div class="col-12">
            ...
            <p>
                <?php $store_id = auth('admin')->user()->store_id;
                $product_id = $whProduct->product_id;
                $stock = \App\Model\StoreProduct::where('store_id', $store_id)
            ...
            <div class="d-flex justify-content-between">
                ...
                @if (count($offers) > 0)
                    <button class="btn btn-primary" type="button" data-toggle="collapse"
                        data-target="#collapseWidthOffers" aria-expanded="false" aria-controls="collapseWidthOffers">
                        Offers
                    </button>
                @endif
            </div>
            ...
            <div>
                    ...
                    @if ($product_details_array)
                        @foreach ($product_details_array as $key => $warehouse)
                            <div class="discountpricing card card-body p-2 d-block mb-2 w-75">
                                ...
                                <strong
                                    class="text-dark mx-2 d-inline-block">₹{{ @$product_details_array[$i]['offer_price'] }}
                                    <strike>₹{{ @$product_details_array[$i]['market_price'] }}</strike></strong>
                                <span class="bg-danger discountbox px-2 py-1 rounded-sm text-white ">
                                    {{ @$product_details_array[$i]['discount'] }}%</span>
                ...
                <div class="collapse width" id="collapseWidthOffers">
                    ...
                    @foreach ($offers as $offer)
                        @php($offerItem = Helpers::getWhProductOfferItem($product->id, $offer->id))
                        <div class="discountpricing card card-body p-2 d-block mb-2 w-75">
                            {{ @$offer->title }}
                            <?php
                            
                            $disType = $offer->discount_type == 'amount' ? '₹' : '%';
                            $msg = $offerItem->quantity . ' ' . $whProduct->unit->title . ' ' . $product->name . ' in only ₹' . $offerItem->amount;
                            ?>
            ...
            <form id="add-to-cart-form" class="mb-2">
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <div class="product-quantity d-flex align-items-center">
                        <div class="input-group input-group--style-2 pr-3" style="width: 160px;">
                            ...
                            <input type="hidden" id="check_max_qty" name="store_total_stock"
                ...
                <div class="d-flex justify-content-center mt-2">
                    <button class="btn btn-primary addToCart" onclick="addToCart()" type="button"
