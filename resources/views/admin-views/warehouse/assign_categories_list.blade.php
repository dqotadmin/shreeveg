    ...
    <div class="row g-2">
        <div class="col-sm-12 col-lg-12">
            <div class="btn--container justify-content-end m-2">
                <a type="button" href="{{route('admin.warehouse.wh-assign-category-page-create',[$warehouses['id']])}}" class="btn btn--primary">{{translate('Category Assign')}}</a>
            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body pt-sm-0 pb-sm-4">
                            ...
                            <div class="row align-items-end g-4"  style="padding-top: 50px;">
                                <div class="col-sm-4 ">
                                    <label class="form-label" for="">{{ translate('Warehouse Admin Name') }} </label>
                                    @foreach(\App\Model\Admin::orderBy('id',
                                    'DESC')->where('admin_role_id',3)->get() as $admin)
                                    @if($warehouses->warehouse_admin_id ==$admin->id)
                                    <input class="form-control" value="{{$admin['f_name']}} {{$admin['l_name']}}"   readonly>
                                    @endif
                                    @endforeach
                                </div>
                                <div class="col-sm-4 ">
                                    ...
                                    <input type="text" name="name[]" class="form-control" maxlength="255"   value="{{$warehouses->name}}" readonly>
                                </div>
                                <div class="col-sm-4 ">
                                    <label class="form-label"  for="exampleFormControlInput1">{{ translate('Warehouse Code') }}  </label>
                                    <input type="text" name="code[]" class="form-control" readonly  value="{{$warehouses->code}}" id="dataAttributeValue">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <table class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                    ...
                    @foreach($wh_assign_categories as $key => $wh_assign_category)
                        ...
                        <tr>
                            ...
                            <input type="checkbox" name="status[]" onclick="status_change_alert('{{ route('admin.warehouse.wh-assign-category-status', [$wh_assign_category->id, $wh_assign_category->status ? 0 : 1]) }}', '{{ $wh_assign_category->status? translate('you_want_to_disable_this_category'): translate('you_want_to_active_this_category') }}', event)"
